...
The binary reads Apple Unified Log .tracev3 data directly, which is what lets DLEAPP
read the Unified Logs in a Mac extraction without a Mac and without Apple's `log show`. It
is not committed to this repository: it is a third-party compiled artifact, it exists per
platform, and an examiner should be able to see exactly which build went into a release.

Ported from iLEAPP, with the same pinned release and digests.
