...
In iLEAPP, whose fetch script this is, a Windows examiner following the run-from-source
instructions got a hard DIGEST MISMATCH refusal on a byte-identical download: upstream's
Windows .sha256 file is UPPERCASE (PowerShell Get-FileHash style) while Python's
hexdigest() is lowercase, and the comparison was case-sensitive. The values were equal; only the presentation differed.
