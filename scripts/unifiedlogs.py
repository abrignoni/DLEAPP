...
The usual workflow for these logs needs a Mac: rebuild a .logarchive package, generate an
evidence-specific Info.plist (harder since macOS 26.4), run
`log show --style json --info --debug`, and hand DLEAPP a JSON file much larger than the
archive.
...
BINARY_ENV_VAR = 'DLEAPP_UNIFIEDLOG_ITERATOR'
...
# record it cannot decode; a handful is normal and thousands would drown the DLEAPP log.
...
# A Mac's Data volume is firmlinked into the system volume, so a logical extraction can hold
# one log store twice, under private/var/db and under System/Volumes/Data/private/var/db, each
# copied at a different moment.
_DATA_VOLUME = ('System', 'Volumes', 'Data')


def _without_data_volume(components):
    for index in range(len(components) - 2):
        if tuple(components[index:index + 3]) == _DATA_VOLUME:
            return components[:index] + components[index + 3:]
    return components


def store_copies(files_found, root):
    """`root` and every other directory with files beneath it that is the same store reached
    through the Data volume's firmlink, that is the same path with or without
    System/Volumes/Data. `root` comes first, then the others by how many files they hold."""
    if not root:
        return []
    root_components = _path_components(root)
    name = root_components[-1]
    key = _without_data_volume(root_components)
    counts = {}
    for path in files_found:
        ...
        for index, component in enumerate(components[:-1]):
            if component == name:
                ...
                if _without_data_volume(candidate) == key:
                    joined = '/'.join(candidate)
                    counts[joined] = counts.get(joined, 0) + 1
    others = sorted((candidate for candidate in counts if candidate != root),
                    key=lambda candidate: (-counts[candidate], candidate))
    return [root] + others


def _extends(shorter, longer, chunk_size=1 << 20):
    """True when the file at `longer` begins with every byte of the file at `shorter`."""
    with open(shorter, 'rb') as short_file, open(longer, 'rb') as long_file:
        while True:
            block = short_file.read(chunk_size)
            if not block:
                return True
            if long_file.read(len(block)) != block:
                return False


def _pick_copy(paths, summary):
    """The copy of one file to read: the one whose bytes begin with every other copy's."""
    chosen = paths[0]
    for other in paths[1:]:
        chosen_size, other_size = os.path.getsize(chosen), os.path.getsize(other)
        if chosen_size == other_size and _extends(other, chosen):
            summary['identical'] += 1
        elif chosen_size < other_size and _extends(chosen, other):
            chosen = other
            summary['extended'] += 1
        elif other_size < chosen_size and _extends(other, chosen):
            summary['extended'] += 1
        else:
            summary['disagreed'] += 1
    return chosen
...
def assemble_merged_archive(diagnostics_dirs, uuidtext_dirs, workdir):
    """assemble_archive() over several copies of one store taken at different moments.

    A file only one copy holds is taken from that copy. A file several copies hold is taken
    from the copy whose bytes begin with every other copy's bytes, the later capture of a
    file the log store has kept appending to; when no copy extends the others, the first
    copy's file is kept and the name is counted as disagreeing. Returns the workdir and a
    summary: files per copy read, and how many names were identical, extended or
    disagreeing across copies.
    """
    os.makedirs(workdir, exist_ok=True)
    summary = {'identical': 0, 'extended': 0, 'disagreed': 0, 'only_in': {}}
    for source_dirs in (diagnostics_dirs, uuidtext_dirs):
        present = [directory for directory in source_dirs
        ...
        for directory in present:
            for root, _, filenames in os.walk(directory):
                ...
                for filename in filenames:
                    ...
                    by_name.setdefault(relative, []).append((directory, os.path.join(root, filename)))
        for relative, copies in sorted(by_name.items()):
            destination = os.path.join(workdir, relative)
            if os.path.exists(destination):
                continue
            if len(copies) == 1:
                summary['only_in'][copies[0][0]] = summary['only_in'].get(copies[0][0], 0) + 1
                source = copies[0][1]
            else:
                source = _pick_copy([path for _, path in copies], summary)
...
def _report_parser_diagnostics(stderr_file):
    """Summarize what the parser complained about, without flooding the DLEAPP log."""
    stderr_file.seek(0)
    reported = 0
    total = 0
