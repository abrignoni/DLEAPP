...
__artifacts_v2__ = {
    "defenderDetections": {
        ...
        "description": "Microsoft Defender Antivirus detection, action and "
                       "quarantine events from the Defender Operational log, with the threat name, "
                       "severity, path, process, detection source and action where the record carries "
                       "them.",
        "author": "@AlexisBrignoni, Claude",
        ...
        "notes": "Read from Microsoft-Windows-Windows Defender%4Operational.evtx, named in "
                 ...
                 "Defender, which is also tried for a message the first copy does not hold. The "
                 "text is the message as FormatMessage prints it, without its "
                 ...
                 "ends with one. A reference neither copy resolves is reported as stored. The "
                 ...
                 "Status Code, Status Description, State, Execution Name, Pre Execution Status "
                 "and Post Clean Status fields of 1116 to 1119, the Remediation User of 1116 and "
                 "the Detection User of 1117 to 1119, and on 1015 the process ID, security "
                 "intelligence ID, "
    ...
    "defenderProtectionChanges": {
        ...
        "notes": "Read from Microsoft-Windows-Windows Defender%4Operational.evtx, named in "
                 ...
                 "Detail are blank. Every value is reported as stored, without any spaces or "
                 "line breaks it begins or ends with, except that an Old Value "
                 "or New Value holding only a parameter reference, %%n, the identifier of a "
                 "message in the provider's parameter file (Microsoft Learn, 'ProviderType "
                 "complex type', "
                 "https://learn.microsoft.com/en-us/windows/win32/wes/eventmanifestschema-providertype-complextype), "
                 "which the Defender provider's registration in the SOFTWARE hive of each "
                 "registered Windows disk image names as MpEvMsg.dll, "
                 ...
                 "Defender, which is also tried for a message the first copy does not hold, and "
                 "otherwise as stored. No row on the registered images holds a "
    ...
    "defenderScans": {
        ...
        "description": "Microsoft Defender Antivirus scan started, finished and stopped "
                       "events from the Defender Operational log, with the scan type, parameters and "
        ...
        "notes": "Read from Microsoft-Windows-Windows Defender%4Operational.evtx, named in "
                 ...
                 "registered images. Every value is reported as stored, without any spaces or "
                 "line breaks it begins or ends with, except a parameter reference, as described "
                 "below. Scan Resources, "
                 ...
                 "text: every scan record on those two images stores %%802 and %%806. "
                 ...
                 "by the reference, for example Antimalware "
                 ...
                 "else the copy under Program Files/Windows Defender, which is also tried for a "
                 "message the first copy does not hold. The text is the message as "
