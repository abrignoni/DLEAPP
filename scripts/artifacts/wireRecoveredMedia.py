__artifacts_v2__ = {
    "wireRecoveredMedia": {
        ...
        "notes": "Only assets still present in an on-disk cache can be recovered. Timestamp is "
                 ...
                 "LevelDB folder and each cache file an asset was recovered from. SHA-256 "
                 "Verified is Yes on every row by construction, because a cached blob is "
                 "decrypted only when its SHA-256 matches an asset-add event, and on wire_win "
                 "Account held one value on all 7 rows. "
