...
# Microsoft\Windows\CurrentVersion\Run and RunOnce, with 32-bit-on-64-bit
# copies under Wow6432Node, and the per-user entries in each NTUSER.DAT under
...
_SOFTWARE_KEYS = (
    ...
    ('Run (Wow6432Node)', r"Microsoft\Wow6432Node\Windows\CurrentVersion\Run"),
    ('RunOnce (Wow6432Node)', r"Microsoft\Wow6432Node\Windows\CurrentVersion\RunOnce"),
)
_NTUSER_KEYS = (
...
__artifacts_v2__ = {
    "runKeys": {
        ...
        "category": "Windows",
        "notes": "One row per value in the Windows Run and RunOnce autostart "
                 "keys. The machine-wide keys are read from the SOFTWARE hive "
                 "under Microsoft\\Windows\\CurrentVersion\\Run and RunOnce, "
                 "including the Wow6432Node copies that hold the 32-bit view on "
                 "a 64-bit system, and the per-user keys from each NTUSER.DAT "
                 "under Software\\Microsoft\\Windows\\CurrentVersion\\Run and "
                 "RunOnce. The hive is named in Source File. Name is the value "
                 "name as stored and Command is the value data as stored, "
                 "neither interpreted. Key records which key an entry came "
                 "from, Run or RunOnce, with (Wow6432Node) appended for the "
                 "32-bit copies; on an image whose entries all sit in one key "
                 "the Key column is constant. Scope is Machine for the SOFTWARE "
                 "hive and User for an NTUSER.DAT, and is constant on an image "
                 "whose entries all come from one of the two. User is taken "
                 "from the NTUSER.DAT path and is blank on every machine-wide "
                 "row. By default a RunOnce value is deleted before its command "
                 "runs, so that key is often empty at acquisition time and a "
                 "hive can contribute no RunOnce rows. These are common "
                 "autostart and persistence locations: the presence of an entry "
                 "does not establish that the program ran, and the absence of an "
                 "entry is not evidence that a program was not set to start "
                 "automatically. Reading the hives requires the python-registry "
                 "package. The parser reads offline hives and does not replay "
                 "their transaction logs (.LOG1/.LOG2). Key layout and the "
                 "RunOnce deletion behavior: Microsoft, 'Run and RunOnce "
                 "Registry Keys', https://learn.microsoft.com/en-us/windows/"
                 "win32/setupapi/run-and-runonce-registry-keys. These keys as a "
                 "persistence location: MITRE ATT&CK T1547.001, "
