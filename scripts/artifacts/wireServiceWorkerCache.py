__artifacts_v2__ = {
    "wireServiceWorkerCache": {
        ...
        "notes": "Reads Chromium Simple Cache entry files (*_0) whose bytes contain wire.com and "
                 ...
                 "responses that are not assets, so the encrypted-body note is shown only where "
                 "an /assets/ URL was resolved. On wire_win every entry resolved one, so Note "
                 "held one value on all 7 rows. 'Response Content Type' is the "
                 ...
                 "On wire_win Response Content Type held one value on all 7 rows, "
                 "application/octet-stream. 'CDN Expires' "
