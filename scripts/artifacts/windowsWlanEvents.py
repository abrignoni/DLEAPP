...
__artifacts_v2__ = {
    "wlanConnections": {
        ...
        "notes": "Read from Microsoft-Windows-WLAN-AutoConfig%4Operational.evtx, named in "
                 ...
                 "Microsoft-Windows-WLAN-AutoConfig.xml#L568-L607, #L643-L675 and "
                 "#L970-L1034). SSID, BSS Type and Connection ID are SSID, BSSType and "
                 "ConnectionId; Profile Name, Connection Mode and PHY Type are ProfileName, "
                 "ConnectionMode and PHYType, which 8001 carries (8003 carries the first "
                 "two); Authentication and Encryption are AuthenticationAlgorithm and "
                 "CipherAlgorithm on 8001 and Auth and Cipher on 11000; Disconnect Reason is "
                 "the Reason text of 8003; Local MAC Address is LocalMac on 11000 and 11001, "
                 "which the manifest's messages label 'Local MAC Address'; Adapter and "
                 "Interface GUID are InterfaceDescription and InterfaceGuid on 8001 and "
                 "8003 and Adapter and DeviceGuid on 11000 and 11001. Every value is "
                 "reported as stored. None of these events carries a BSSID field in the "
                 "manifest, so no access point hardware address is reported. SSID, BSS "
                 "Type, Adapter and Interface GUID each held one value on every row of "
                 "pc_mus_001_win11 and lonewolf_win10. Disconnect Reason is empty on "
                 "lonewolf_win10, which carries no 8003 records. Event Time (UTC) is the "
                 ...
                 "record stores: it held one value on every row of pc_mus_001_win11 and two "
                 "values on lonewolf_win10. af_case2_win10 carries no WLAN-AutoConfig "
                 "Operational log. Not reported: the log's other events (8000, 11004, 11005 "
                 "and 11010 on the registered images). A record python-evtx cannot render, "
                 "or whose XML does not parse, is counted in the run log and not reported; "
                 "every record in this log rendered on the registered images. Reading needs "
