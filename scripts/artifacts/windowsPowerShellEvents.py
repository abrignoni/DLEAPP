...
__artifacts_v2__ = {
    ...
    "powershellConsoleEvents": {
        ...
        "notes": "Read from Microsoft-Windows-PowerShell%4Operational.evtx, named in the "
                 "report's located-at line; only Microsoft-Windows-PowerShell records with "
                 "Event ID 40961, 40962 or 53504 are read. Event is the message PowerShell's "
                 "instrumentation manifest gives each: 40961 (0xA001) 'PowerShell console is "
                 "starting up', 40962 (0xA002) 'PowerShell console is ready for user input' "
                 "and 53504 (0xD100) 'PowerShell has started an IPC listening thread on "
                 "process: %1 in AppDomain: %2' (https://github.com/PowerShell/PowerShell/"
                 "blob/0817ada8e7b95717fda4483054ee8ed0f1367ac8/src/"
                 "PowerShell.Core.Instrumentation/PowerShell.Core.Instrumentation.man"
                 "#L1746-L1767, #L1091-L1102, #L5371-L5376 and #L5823-L5824; PowerShell 7's "
                 "manifest, whose event IDs and 53504 fields param1 and param2 the tested "
                 "records carry). Process ID is the process ID in the record's Execution "
                 "element, and for 53504 its param1, which equalled the Execution process ID "
                 "on all 50 53504 rows on the registered images. AppDomain is the 53504 "
                 "param2 and is blank on 40961 and 40962 rows; it held DefaultAppDomain on "
                 "every 53504 row on the registered images. User SID is the SID in the "
                 "record's Security element and held one value on every row of "
                 "af_case2_win10. Event Time (UTC) is the record's TimeCreated SystemTime, "
                 "which python-evtx renders from the FILETIME the record stores, counted in "
                 "UTC (python-evtx 0.8.1, https://github.com/williballenthin/python-evtx/"
                 "blob/cab997af04b6caae68b306e5c2c40b3aa751454e/Evtx/BinaryParser.py"
                 "#L105-L113). Record ID is the record's EventRecordID. Computer is the "
                 "machine name the record stores: it held one value on every row of "
                 "af_case2_win10 and pc_mus_001_win11 and two values on lonewolf_win10. A "
                 "record python-evtx cannot render, or whose XML does not parse, is counted in "
                 "the run log and not reported; every record in this log rendered on the "
                 "registered images. The other records in this log are 4104 (the PowerShell "
                 "Script Blocks artifact) and 4103 (the PowerShell Pipeline Execution Details "
                 "artifact). Reading needs the python-evtx package (pip install python-evtx).",
        "paths": ("*/Windows/System32/winevt/Logs/Microsoft-Windows-PowerShell%4Operational.evtx",),
        "output_types": ["standard"],
        "artifact_icon": "terminal",
        "sample_data": {
