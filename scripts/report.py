...
from scripts.version_info import dleapp_version, dleapp_contributors
...
def create_index_html(reportfolderbase, time_in_secs, time_HMS, extraction_type, image_input_path, nav_list_data, casedata):
    ...
    authors_data = generate_authors_table_code(dleapp_contributors)
    credits_code = credits_block.format(authors_data)

    # WRITE INDEX.HTML LAST
    filename = 'index.html'
    page_title = 'DLEAPP Report'
    body_heading = 'Drones Logs Events And Properties Parser'
    body_description = 'DLEAPP is an open source project that aims to parse drone extractions for the purpose of triage analysis.'
    active_nav_list_data = mark_item_active(nav_list_data, filename) + nav_bar_script
...
def generate_authors_table_code(dleapp_contributors):
    authors_data = ''
    for author_name, blog, tweet_handle, git in dleapp_contributors:
        author_data = ''
        if blog:
