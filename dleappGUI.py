...
layout = [  [sg.Text('Drones Logs, Events, And Properties Parser', font=("Helvetica", 22))],
            [sg.Text('https://github.com/abrignoni/DLEAPP', font=("Helvetica", 14))],
            [sg.Frame(layout=[
                    [sg.Input(size=(97,1)), 
                ...
                title='Select a file (tar/zip/gz) or directory of the target drone extraction for parsing:')],
            [sg.Frame(layout=[
...
while True:
    ...
    if event == "DESELECT ALL":  
         # none modules
        for x in range(MODULE_START_INDEX, module_end_index):
            window[x].Update(False if window[x].metadata != '' else True)  
